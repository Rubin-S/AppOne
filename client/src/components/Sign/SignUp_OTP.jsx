...
const SignUpPage = ({ setCurrentStep }) => {

  const {signupData, setSignupData} = useAppContext(); // Accessing context to manage login state

  const phoneNumber = signupData.phone; // Using phone number from context
  const setPhoneNumber = (number) => {
    setSignupData((prev) => ({ ...prev, phone: number })); // Update phone number in context
  };

  const isVerified = signupData.phoneVerified; // Using  OTP verification from context
  const setIsVerified = (isVerified) => {
    setSignupData((prev) => ({ ...prev, phoneVerified: isVerified })); // Update OTP status in context
  };
    ...
    if (otpCode.length === otp.length) {
      ...
      setTimeout(() => {
        if (otpCode === "1234") {
          setIsVerified(true);
          setCurrentStep(1);
          setShowOTPVerification(false); // Close OTP verification popup
          setError("");
  ...
  return (
    <GoogleOAuthProvider clientId="ENTER THE CLIENT ID HERE ">
      <div className="max-w-md my-auto p-6 space-y-6">
        ...
        <div>
          ...
          <form onSubmit={handlePhoneNumberSubmit} className="flex gap-2 mb-4">
            ...
            <input
              ...
              required
            />
            <button type="submit" className="bg-primary rounded-full px-4 py-2 text-white">
